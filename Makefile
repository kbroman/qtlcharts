...
CHARTEXAMPLES = $(CHARTEX)/iboxplot.html $(CHARTEX)/iplotCorr.html \
				...
				$(CHARTEX)/itimeplot.html $(CHARTEX)/idotplot.html \
				$(CHARTEX)/scat2scat.html $(CHARTEX)/itriplot.html \
				$(CHARTEX)/iplot.html
...
$(CHARTEX)/iplot.html: $(CHARTEX)/R/iplot_example.R $(CHARTEX)/R/itimeplot_data.rds
	cd $(<D); R CMD BATCH --no-save $(<F)
