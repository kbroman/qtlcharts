...
`tipdirection = NULL` &mdash; direction of tool tips (east/west/north/south)
...
`scat_title = ""` &mdash; title of scatterplot panel
...
`yNA = list(handle=false,force=false,width=15,gap=10)` &mdash; treatment of missing phenotype values in right panels
...
`gridlines = 0` &mdash; number of grid lines

`gridcolor = "white"` &mdash; color of grid lines

`gridwidth = 1` &mdash; width of grid lines in pixels
...
`xlab2 = "X"` &mdash; x-axis label for right panel (can be a vector, with separate values for each dataset in `scat2data`)

`ylab2 = "Y"` &mdash; y-axis label for right panel (can be a vector, with separate values for each dataset in `scat2data`)
