...
requires installation of the packages [R/qtl](http://www.rqtl.org) and
...
If you want a caption below the chart, use `caption`. Similarly, you
can use `heading` to get an `<h2>` heading above the plot, and
`footer` to add a footer below. These is intended
largely for charts [saved to a
file](http://localhost:4000/assets/vignettes/userGuide.html#saving_to_a_file)
with `htmlwidgets::saveWidget()`.

```{r iplotCorr_caption, eval=FALSE}
iplotCorr(expr, geno, reorder=TRUE,
          chartOpts=list(scatcolors=c("lightblue", "lightgreen", "pink"),
                         heading="iplotCorr example",
...
[Mouse Phenome Database](http://phenome.jax.org/db/q?rtn=projects/projdet&reqprojid=282).
