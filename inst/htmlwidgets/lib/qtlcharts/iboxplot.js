...
iboxplot = function iboxplot(widgetdiv, data, chartOpts) {
  ...
  halfheight = height / 2; // y-axis limits for top figure
  ...
  topylim[1] = Math.ceil(topylim[1]); // y-axis limits for bottom figure
  ...
  indindex = d3.range(data.ind.length); // adjust counts object to make proper histogram
  ...
  midQuant = (nQuant + 1) / 2 - 1; // x and y scales for top figure

  xScale = d3.scaleLinear().domain([-1, data.ind.length]).range([margin.left, width - margin.right]); // width of rectangles in top panel

  recWidth = xScale(1) - xScale(0);
  yScale = d3.scaleLinear().domain(topylim).range([halfheight - margin.bottom, margin.top]); // function to create quantile lines
  ...
  svg = d3.select(widgetdiv).select("svg").append("svg").attr("width", width).attr("height", halfheight).attr("class", "qtlcharts"); // gray background

  svg.append("rect").attr("x", margin.left).attr("y", margin.top).attr("height", halfheight - margin.top - margin.bottom).attr("width", width - margin.left - margin.right).attr("stroke", "none").attr("fill", rectcolor).attr("pointer-events", "none"); // axis on left

  LaxisData = yScale.ticks(6);
  Laxis = svg.append("g").attr("id", "Laxis"); // axis: white lines
  ...
  Laxis.append("g").selectAll("empty").data(LaxisData).enter().append("line").attr("class", "line").attr("class", "axis").attr("x1", margin.left).attr("x2", width - margin.right).attr("y1", function (d) {
    ...
    return yScale(d);
  }).attr("stroke", "white").attr("pointer-events", "none"); // axis: labels
  ...
  Laxis.append("g").selectAll("empty").data(LaxisData).enter().append("text").attr("class", "axis").text(function (d) {
    ...
    return yScale(d);
  }).attr("dominant-baseline", "middle").attr("text-anchor", "end"); // axis on bottom

  BaxisData = xScale.ticks(10);
  Baxis = svg.append("g").attr("id", "Baxis"); // axis: white lines
  ...
  Baxis.append("g").selectAll("empty").data(BaxisData).enter().append("text").attr("class", "axis").text(function (d) {
    ...
    return xScale(d - 1);
  }).attr("dominant-baseline", "middle").attr("text-anchor", "middle"); // colors for quantile curves
  ...
  }).attr("fill", "purple").attr("stroke", "none").attr("opacity", "0").attr("pointer-events", "none"); // vertical rectangles representing each array
  ...
  longRect = indRectGrp.selectAll("empty").data(indindex).enter().append("rect").attr("x", function (d) {
    return xScale(d) - recWidth / 2;
  }).attr("y", margin.top).attr("width", recWidth).attr("height", halfheight - margin.top - margin.bottom).attr("fill", "purple").attr("stroke", "none").attr("opacity", "0"); // label quantiles on right
  ...
  rightAxis.selectAll("empty").data(data.qu).enter().append("text").attr("class", "qu").text(function (d) {
    ...
    return qucolors[i];
  }).attr("text-anchor", "end").attr("dominant-baseline", "middle"); // box around the outside

  svg.append("rect").attr("x", margin.left).attr("y", margin.top).attr("height", halfheight - margin.top - margin.bottom).attr("width", width - margin.left - margin.right).attr("stroke", "black").attr("stroke-width", 2).attr("fill", "none"); // lower svg
  ...
  lowyScale = d3.scaleLinear().domain([0, botylim[1] + 1]).range([halfheight - margin.bottom, margin.top]); // gray background

  lowsvg.append("rect").attr("x", margin.left).attr("y", margin.top).attr("height", halfheight - margin.top - margin.bottom).attr("width", width - margin.left - margin.right).attr("stroke", "none").attr("fill", rectcolor); // axis on left

  lowBaxisData = lowxScale.ticks(8);
  lowBaxis = lowsvg.append("g").attr("id", "lowBaxis"); // axis: white lines
