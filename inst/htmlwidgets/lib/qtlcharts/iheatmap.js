...
iheatmap = function iheatmap(widgetdiv, data, chartOpts) {
  ...
  hbot = height - htop;
  wright = width - wleft; // Select the svg element
  ...
  z_transpose = d3panels.transpose(data.z); //# configure the three charts
  ...
  if (flip_vert_slice) {
    // flip the vertical slice (top-right panel)
    var _ref = [ver_opts.ylab, ver_opts.xlab];
  ...
  verslice = d3panels.panelframe(ver_opts); //# now make the actual charts
  // heatmap

  g_heatmap = svg.append("g").attr("id", "heatmap");
  myheatmap(g_heatmap, data); // horizontal slice (below)

  g_horslice = svg.append("g").attr("id", "horslice").attr("transform", "translate(0," + htop + ")");
  horslice(g_horslice); // vertical slice (to the right)
