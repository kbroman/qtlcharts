...
iplotMScanone_noeff = function(widgetdiv, lod_data, times, chartOpts) {
  var altrectcolor, axispos, chartdivid, chrGap, colors, g_heatmap, g_lodchart, g_lodvphe, hbot, height, htop, i, linecolor, linewidth, lod_ylab, lodchart_curve, lodvphe, lodvphe_curve, margin, mylodchart, mylodheatmap, nullcolor, nxticks, plotLodCurve, plot_lodvphe, pointcolor, pointsize, pointstroke, rectcolor, ref, ref1, ref10, ref11, ref12, ref13, ref14, ref15, ref16, ref17, ref18, ref19, ref2, ref20, ref21, ref22, ref3, ref4, ref5, ref6, ref7, ref8, ref9, svg, titlepos, widgetdivid, width, wleft, wright, x, xlim, xticks, zlim, zmax, zthresh;
  ...
  g_lodchart = svg.append("g").attr("transform", "translate(0," + htop + ")").attr("id", "lodchart");
  ...
  plotLodCurve = function(lodcolumn) {
    if (lodchart_curve != null) {
      lodchart_curve.remove();
    }
    ...
    return lodchart_curve(mylodchart, {
      chr: lod_data.chr,
      pos: lod_data.pos,
      marker: lod_data.marker,
      lod: (function() {
  ...
  g_lodvphe = svg.append("g").attr("transform", "translate(" + wleft + ",0)").attr("id", "curvechart");
  lodvphe(g_lodvphe);
  lodvphe_curve = null;
  plot_lodvphe = function(posindex) {
    if (lodvphe_curve != null) {
      lodvphe_curve.remove();
    }
    lodvphe_curve = d3panels.add_curves({
      linecolor: linecolor,
      linewidth: linewidth
    });
    return lodvphe_curve(lodvphe, {
      x: [x],
      y: [
        (function() {
          var results;
  ...
  return mylodheatmap.cells().on("mouseover", function(d) {
    ...
    if (times == null) {
      return g_lodvphe.select("text#xaxis" + d.lodindex).attr("opacity", 1);
    }
  }).on("mouseout", function(d) {
    g_lodchart.select("g.title text").text("");
    g_lodvphe.select("g.title text").text("");
    if (times == null) {
      return g_lodvphe.select("text#xaxis" + d.lodindex).attr("opacity", 0);
    }
  });
};
