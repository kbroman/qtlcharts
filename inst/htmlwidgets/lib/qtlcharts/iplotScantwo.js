...
iplotScantwo = function iplotScantwo(widgetdiv, scantwo_data, pheno_and_geno, chartOpts) {
  ...
  svg = div.select("svg"); // force chrnames to be a list

  scantwo_data.chrnames = d3panels.forceAsArray(scantwo_data.chrnames);
  scantwo_data.nmar = d3panels.forceAsArray(scantwo_data.nmar); // size of heatmap region
  ...
  wbot = width / 2; // selected LODs on left and right

  leftvalue = "int";
  rightvalue = "fv1"; // keep track of chromosome heatmap selections

  cur_chr1 = cur_chr2 = ''; // cicolors: check they're the right length or construct them
  ...
  if (pheno_and_geno != null) {
    ...
    if (cicolors != null) {
      ...
      if (n < ncat) {
        ...
        cicolors = function () {
          var l, ref34, results; // not provided; select them
  ...
  mylod2dheatmap(g_heatmap, scantwo_data); // function to add tool tips and handle clicking

  add_cell_tooltips = function add_cell_tooltips() {
    d3panels.tooltip_text(mylod2dheatmap.celltip(), function (d) {
    ...
    return mylod2dheatmap.cells().on("click", function (d) {
      ...
      plot_scan(d.yindex, 1, 1, rightvalue); // plot the effect plot and phe x gen plot to right
  ...
  add_cell_tooltips(); // to hold groups and positions of scan and effect plots
  ...
  plot_effects = function plot_effects(markerindex1, markerindex2) {
    ...
    gn1 = [];
    gn2 = [];
    cicolors_expanded = []; // need to deal separately with X chr
    ...
    if (cur_chr1 !== chr1 || cur_chr2 !== chr2) {
      ...
      mydotchart(g_eff[1], pxg_data); // revise point colors
      ...
      d3.select("#xaxislab1").remove(); // grab scale and get info to take inverse
    ...
    effcharts = [mycichart, mydotchart]; // add second row of labels
...
lod_for_heatmap = function lod_for_heatmap(scantwo_data, left, right) {
  var i, j, l, o, ref, ref1, thelod, z; // make copy of lod
