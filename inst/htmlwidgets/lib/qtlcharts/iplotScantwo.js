...
// not provided; select them
  ...
  add_cell_tooltips = function() {
    ...
    return mylod2dheatmap.cells().on("click", function(event, d) {
      var mari, marj;
      mari = scantwo_data.marker[d.xindex];
      marj = scantwo_data.marker[d.yindex];
      if (d.xindex === d.yindex) { // skip the diagonal case
        return null;
      }
    ...
    if (cur_chr1 !== chr1 || cur_chr2 !== chr2) {
      ...
      mydotchart = d3panels.dotchart({
        ...
        xcategories: (function() {
          var results = [];
          for (var v = 1, ref41 = gn1.length; 1 <= ref41 ? v <= ref41 : v >= ref41; 1 <= ref41 ? v++ : v--){ results.push(v); }
          return results;
        }).apply(this),
        xcatlabels: gn1,
        xlab: "",
      ...
      (function() {
        var results = [];
        for (var v = 0; v <= 30; v++){ results.push(v); }
        return results;
