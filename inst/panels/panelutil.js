...
var abs, calc_crosstab, chrscales, ci_by_group, colSums, displayError, expand2vector, forceAsArray, formatAxis, getLeftRight, log10, log2, matrixExtent, matrixMax, matrixMaxAbs, matrixMin, maxdiff, mean_by_group, median, missing2null, pullVarAsArray, reorgLodData, rowSums, sd_by_group, selectGroupColors, sumArray, transpose, unique;
...
ci_by_group = function(g, y, m) {
  ...
  for (i in means) {
    ci[i] = {
      mean: means[i],
      low: sds[i] != null ? means[i] - m * sds[i] : means[i],
      high: sds[i] != null ? means[i] + m * sds[i] : means[i]
    };
  }
  return ci;
};
