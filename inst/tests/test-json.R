...
test_that("test simple conversions to JSON", {
  ...
  output01 <- "{ \"x\" : [ \"a\" ], \"y\" : [ null ] }"
  expect_equal(toJSON(input01), output01)

  input02 <- list(x=NA, y="a")
  output02 <- "{ \"x\" : [ null ], \"y\" : [ \"a\" ] }"
  expect_equal(toJSON(input02), output02)

  input03 <- list(x=NA, y=1)
  output03 <- "{ \"x\" : [ null ], \"y\" : [ 1 ] }"
  expect_equal(toJSON(input03), output03)

  input04 <- list(x=1, y=NA)
  output04 <- "{ \"x\" : [ 1 ], \"y\" : [ null ] }"
  expect_equal(toJSON(input04), output04)

  input05 <- c(x="a", y=NA)
  output05 <- "[ \"a\", null ]"
  expect_equal(toJSON(input05), output05)

  input06 <- c(x=NA, y="a")
  output06 <- "[ null, \"a\" ]"
  expect_equal(toJSON(input06), output06)

  input07 <- c(x=1, y=NA)
  output07 <- "[ 1, \"NA\" ]"
  expect_equal(toJSON(input07), output07)

  input08 <- c(x=NA, y=1)
  output08 <- "[ \"NA\", 1 ]"
  expect_equal(toJSON(input08), output08)
