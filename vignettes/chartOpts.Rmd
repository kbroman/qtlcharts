...
`axispos = {xtitle:25, ytitle:30, xlabel:5, ylabel:5}    ` &mdash; position of axis labels in pixels (xtitle, ytitle, xlabel, ylabel)
...
`corcolors = ["darkslateblue", "white", "crimson"]    ` &mdash; heat map colors (same length as `zlim`)

`zlim = [-1, 0, 1]         ` &mdash; z-axis limits
...
`axispos = {xtitle:25, ytitle:30, xlabel:5, ylabel:5}     ` &mdash; axis positions in heatmap
...
`colors = ["crimson", "white", "slateblue"]                ` &mdash; colors for heat map

`lodlim = [0, 12]            ` &mdash; range of LOD values to display; omit below 1st, truncate about 2nd
