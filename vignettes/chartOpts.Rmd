...
`hbot = 300` &mdash; height (in pixels) of each of the lower panels

`margin = list(left=60, top=30, right=10, bottom=40, inner=5)` &mdash; margins in each panel

`axispos = list(xtitle=25, ytitle=30, xlabel=5, ylabel=5)` &mdash; axis positions in heatmap

`lightrect = "#e6e6e6"` &mdash; background color in heatmap; light rect in lower panels
...
`colors = c("white", "slateblue")` &mdash; colors for heat map
