...
`margin = list(left=60, top=40, right=40, bottom=40)` &mdash; margins in each panel

`axispos = list(xtitle=25, ytitle=30, xlabel=5, ylabel=5)` &mdash; axis positions in heatmap

`rectcolor = "#e6e6e6"` &mdash; background color in heatmap and crosstab

`hilitcolor = "#e6e6e6"` &mdash; highlight color in crosstab

`nullcolor = "#e6e6e6"` &mdash; color of null pixels in heat map

`bordercolor = "black"` &mdash; border color in heat map and in cross-tab

`colors = c("slateblue", "white", "crimson")` &mdash; colors for heat map

`lodlim = c(2, 12)` &mdash; range of LOD values to display; omit below 1st, truncate about 2nd

`rflim = c(0.01, 0.4)` &mdash; range of rf values to display (will also show symmetric interval on other side of 1/2, in another color)

`lodonly = false` &mdash; if true, show symmetric plot of LOD scores

`rfonly = false` &mdash; if true, show symmetric plot of rec frac
