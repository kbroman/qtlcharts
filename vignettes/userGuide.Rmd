...
If you want to add a title but remove the caption, you could do the following:
...
report. See the separate [`Rmarkdown` vignette](http://kbroman.org/qtlcharts/assets/vignettes/Rmarkdown.html):
...
There are drop-down
menus to select different which LOD scores to display in the heat map.
Hover over the heatmap to view the LOD scores, and click on a pixel to
...
`pixelPerCell`, the number of pixels, in each direction, for each
small rectangle (&ldquo;cell&rdquo;) within the heatmap.
...
the lower right (so that chromosome 1 is at the top on the left rather
